  ...
  module Encryption
    ...
    class RC4
      ...
      def mix_table
        ...
        i = @mix_table[@i]
        @mix_table[@i] = @mix_table[@j]
        @mix_table[@j] = i

        @mix_table[(@mix_table[@i] + @mix_table[@j]) % 256]
      end
      ...
      def decipher(str)
        str.each_char.each_slice(2).map(&:join).map do |hex|
          (hex.hex ^ mix_table).chr
        end.join
      end

      def public_key_hash(public_key)
        half = public_key.length / 2
        table, key = public_key.slice(0..(half - 1)), public_key.slice(half..-1)
        i = 0
        key.each_char.each_with_index do |chr, idx|
          pos = table.index(chr) || -1
          pos *= 2 if pos % 2 == 0
          pos *= 3 if idx % 3 == 0
          pos = table.length % 2 if idx < 0
          i += pos
          i ^= (pos << (i % 3) * 8)
        end
        i
